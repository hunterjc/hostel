...
//requiring from env end
function createNewStudent(req, res) {
  const { name, personalNumber, guardianNumber, hostelID, password, roomNo } =
    req.body;
  ...
  newStudentUser
    .save()
    .then(() => {
      const accessToken = jwt.sign({ id: newStudentUser._id }, token);
...
function login(req, res) {
  ...
  studentModel
    .findOne({ personalNumber: personalNumber })
    .then((studentUser) => {
      if (!studentUser) {
...
function view(req, res) {
  studentModel
    ...
    .then((studentPointer) => {
      ...
      adminModel
        .findById(studentPointer.hostelID)
        .then((adminPointer) => {
          if (!adminPointer) {
            console.log(
              `Invalid hostel ID saved in database for the student with id ${studentUser._id.toString()}`
            );
            res.status(500);
            res.json({
